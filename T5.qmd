...
$$= 0{,}0000\,0000\,0000\,0000\,0000\,0001\,111\ldots \times 2^{10} \approx 1{,}111_2 \times 2^{-15} $$

$$= (1 + 0{,}5 + 0{,}25 + 0{,}125) / 2 \times 10^{5} = \frac{1{,}875}{32768} = 5{,}722 \times 10^{-5}$$

$$= (1 + 0{,}5 + 0{,}25 + 0{,}125) \times 2^{-15} = \frac{1{,}875}{32768} = 5{,}722 \times 10^{-5}$$

**Solució**: 

$$\varepsilon = 5{,}722 \times 10^{-5}$$
:::
...
$$1\,0000\,0010\,1000_2 = 2^{12} + 2^5 + 2^3 + 2^2 = 4096 + 32 + 8 = 4136$$
...
Si el resultat d'una operació té un exponent $e > 127$, no és representable i es produeix una excepció d'***overflow***.
...
### Error de representació i underflow {#sec-error-representacio}
...
Per a un nombre normalitzat qualsevol de la forma $v_0 = m \times 2^e$, el valor representable següent és $v_1 = (m + 2^{-23}) \times 2^e$, de manera que:

$$\varepsilon_{\max} < 2^{e-23}$$

L'error relatiu màxim s'obté dividint per $v_0$ i tenint en compte que $m \geq 1{,}0$:

$$\eta_{\max} < \frac{2^{e-23}}{m \times 2^e} = \frac{2^{-23}}{m} \leq 2^{-23} = 1\ \text{ULP}$$
...
Quan el maquinari detecta aquesta situació genera una excepció d'***underflow***, que permet al programari tractar-la adequadament. Els nombres **denormals** (@sec-codificacions-especials) són el mecanisme que defineix l'IEEE-754 per reduir aquest error de forma gradual.
:::
...
L'error relatiu ja no està uniformement acotat, però **creix de forma gradual** a mesura que $v$ s'acosta a zero, en lloc d'augmentar de cop com passaria sense denormals. Per aquesta raó el mecanisme es coneix també com ***gradual underflow***.
...
Quan el resultat exacte $v$ d'una operació no és representable, cal triar entre els dos valors representables $v_0 < v < v_1$ més pròxims. Aquesta aproximació s'anomena **arrodoniment**. L'IEEE-754 defineix quatre modes:
...
Els modes d'arrodoniment a $+\infty$ i a $-\infty$ s'usen conjuntament en **aritmètica d'intervals**, una tècnica que acota els errors d'arrodoniment per garantir la fiabilitat dels resultats numèrics.

::: {#tip-arrodoniment-rne .callout-tip}
## Arrodoniment RNE

Arrodoniu les mantisses de 6 bits següents a 3 bits:

| Valor exacte | Bits descartats | Criteri | Resultat |
| :---: | :---: | :--- | :---: |
| $101{,}010$ | $010$ | 1r bit descartat = 0 → a l'anterior | $101$ |
| $101{,}011$ | $011$ | 1r bit descartat = 0 → a l'anterior | $101$ |
| $101{,}101$ | $101$ | 1r bit descartat = 1, resta ≠ 0 → al següent | $110$ |
| $101{,}100$ | $100$ | 1r bit descartat = 1, resta = 0 → empat → al parell ($110$) | $110$ |
| $100{,}100$ | $100$ | 1r bit descartat = 1, resta = 0 → empat → al parell ($100$) | $100$ |
:::

::: {#wrn-bits-guarda .callout-warning collapse=true}
## Bits de guarda: per què 3 bits són suficients

Durant una suma o resta en coma flotant, la mantissa del sumand de menor magnitud es desplaça a la dreta per igualar els exponents. Els bits que queden fora de la mantissa de $p$ bits s'anomenen **bits de guarda**. Descartar-los introdueix errors; conservar-los tots requeriria un sumador de mida arbitrària.

Es demostra que **tres bits de guarda** (anomenats **G**, **R** i **S**) són suficients per obtenir exactament el mateix resultat arrodonit que amb tots els bits:

* **G** (*Guard*): primer bit de guarda; pot passar a la mantissa final després de la normalització.
* **R** (*Round*): segon bit de guarda; determina si el resultat s'arrodoneix al valor següent.
* **S** (*Sticky*): OR lògica de tots els bits restants; distingeix el cas equidistant del cas "al següent".

La regla d'arrodoniment RNE amb tres bits de guarda és:
...
3. **Normalitzar** el resultat (desplaçar la mantissa fins que la part entera sigui 1, ajustant l'exponent en conseqüència). Comprovar *overflow*/*underflow*.
...
**Pas 4. Arrodonir** a 4 dígits (bits descartats en negreta: $\mathbf{580}$; primer dígit descartat $\geq 5$ → al següent):
...
$$= 0{,}0000\,0000\,0000\,0000\,0000\,001 \times 2^1 = 1{,}0_2 \times 2^{-24} \approx 6{,}0 \times 10^{-8}$$
:::
...
4. **Normalitzar** el resultat. Comprovar *overflow*/*underflow*.
...
**Pas 5. Arrodonir** a 4 dígits (bits descartats: $\mathbf{12}$; primer dígit descartat $< 5$ → a l'anterior):
...
\hline
10{,}1101\,1000\,0000\,0000\,0000\,0111\,10
\end{array}$$
...
$$|z| = 1{,}0110\,1100\,0000\,0000\,0000\,0011\,110_2 \times 2^{-2}$$

$e = -2 \in [-126,+127]$: cap excepció.

**Pas 5. Arrodonir** a 24 bits (bits de guarda: $\mathbf{110}$; $G=1$, $R=1$ → al següent):

$$\begin{array}{r}
1{,}0110\,1100\,0000\,0000\,0000\,011 \\
+\hspace{10.3em}1 \\
\hline
1{,}0110\,1100\,0000\,0000\,0000\,100
\end{array} \times 2^{-2}$$

**Pas 6. Codificar** ($S=1$, $E_u = -2 + 127 = 125 = 0111\,1101_2$):

$$z = 1\;|\;0111\,1101\;|\;011\,0110\,0000\,0000\,0000\,0100_2 = \texttt{0xBEB60004}$$
:::
...
| `f10`–`f11` | `fa0`–`fa1` | Arguments / valor de retorn | Caller |
...
A més dels registres `f0`–`f31`, l'extensió F afegeix el registre de control i estat de coma flotant **`fcsr`** (*Floating-Point Control and Status Register*), que controla el mode d'arrodoniment i registra les excepcions produïdes per les operacions de coma flotant.
...
| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
| `feq.s` | `rd, rs1, rs2` | $rd = (rs1 = rs2)\ ?\ 1 : 0$ | *Float Equal* | R |
| `flt.s` | `rd, rs1, rs2` | $rd = (rs1 < rs2)\ ?\ 1 : 0$ | *Float Less Than* | R |
| `fle.s` | `rd, rs1, rs2` | $rd = (rs1 \leq rs2)\ ?\ 1 : 0$ | *Float Less or Equal* | R |
: {tbl-colwidths="[8,18,35,30,9]" .striped}
...
| Mnemònic | Operands | Operació | Nom |
| :--- | :--- | :--- | :--- |
| `fcvt.s.w` | `rd, rs1` | $rd = (float)\ rs1$ | Enter amb signe → flotant |
| `fcvt.s.wu` | `rd, rs1` | $rd = (float)\ rs1$ | Natural → flotant |
| `fcvt.w.s` | `rd, rs1` | $rd = (int)\ rs1$ | Flotant → enter amb signe (trunca) |
| `fcvt.wu.s` | `rd, rs1` | $rd = (unsigned)\ rs1$ | Flotant → natural (trunca) |
