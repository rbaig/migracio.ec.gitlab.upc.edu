...
$$= (1 + 0.5 + 0.25 + 0.125) / 2 \times 10^{5} = 1.875 / 32768 = 5{,}722 \times 10^{-5}$$
...
El resultat exacte $v$ d'una operació pot no ser representable amb un nombre finit de bits de fracció. En aquest cas cal aproximar-lo per un dels dos valors representables més pròxims $v_0 < v < v_1$. L'**error absolut de representació** es defineix com $\varepsilon = |v - v_0|$ i el **error relatiu** com $\eta = \varepsilon\,/\,|v|$.
...
El pròleg i l'epíleg d'una subrutina que usa registres segurs de coma flotant segueix el mateix patró que per als enters (@sec-multinivell-pila):
