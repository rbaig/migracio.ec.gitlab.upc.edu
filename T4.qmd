...
A **EC**, en els càlculs d'adreces de matriu, s'usa `mul` per calcular índexs i adreces. Els 32 bits de més pes s'ignoren perquè qualsevol adreça vàlida en RV32I cap en 32 bits.
:::
...
```{.s filename="RV32I"}
.data
mat:    .space  NF*NC*4             # NF files × NC columnes × 4 bytes
...
$$@\texttt{mat[i][j]} = @\texttt{mat} + (i \cdot \texttt{NC} + j) \cdot T$$
{#eq-matriu-acces-aleatori}
...
Quan un bucle recorre els elements d'un vector o matriu i la distància en bytes entre les adreces de dos elements consecutius del recorregut és constant, es pot aplicar l'**accés seqüencial**.

Sigui un recorregut format per la seqüència d'elements $X_0, X_1, X_2, \ldots$ tal que la diferència entre les adreces de qualsevol parell d'elements consecutius és constant:

$$\text{stride} = @X_{i+1} - @X_i$$

En lloc de calcular l'adreça de cada element a partir de l'adreça base i l'índex:

$$@X_i = @X_0 + i \cdot T$$

es manté un **punter** que s'actualitza sumant-li l'stride a cada iteració:

$$@X_{i+1} = @X_i + \text{stride}$$
...
2. Calcular l'stride restant les adreces de dos elements consecutius. 3. A cada iteració: accedir a l'element desreferenciant `p`, i sumar-li    l'stride per avançar al següent.
...
La infraestructura de control queda en **3 instruccions** per iteració. El salt incondicional `j for` ha desaparegut, substituït pel salt condicional al final `blt`.
:::
