---
title: "Tema 4: Aritmètica entera i matrius"
...
### Condicions d'overflow

Per a la **suma** $s = a + b$, es produeix overflow **si i només si els dos operands són del mateix signe i el resultat és de signe diferent**.

Per a la **diferència** $d = a - b$, es produeix overflow **si i només si** la diferència és del mateix signe que el subtraend $b$ però de signe diferent que el minuend $a$**.

::: {#wrn-overflow-demostracio .callout-warning collapse=true}
## Demostració de les condicions d'overflow en Ca2
...
**Cas 2 — Mateix signe:** la suma exacta ha de ser del mateix signe que els operands, de manera que $S_n = A_{n-1} = B_{n-1}$. La cadena truncada $S' = S_{n-1}\ldots S_0$ és la representació correcta de $s$ si i només si $S_{n-1} = S_n$, és a dir, si $S_{n-1} = A_{n-1} = B_{n-1}$. Si $S_{n-1} \neq A_{n-1}$, el resultat truncat és incorrecte: hi ha overflow.

Per a la diferència $d = a - b$, es pot aplicar la mateixa regla a l'expressió $a = d + b$.
:::

### Detecció de l'overflow per maquinari

En un sumador amb propagació de *carry* de $n$ bits, es produeix overflow quan el *carry* d'entrada al bit de més pes difereix del *carry* de sortida:

$$v = c_{n-1} \oplus c_n$$ {#eq-overflow-hardware}

::: {#wrn-overflow-hardware .callout-warning collapse=true}
## Detecció d'overflow per maquinari: el sumador amb propagació de l'arrossegament
...
Al darrer sumador complet (bit de més pes), el *carry-in* és $c_{n-1}$ i el *carry-out* és $c_n$. L'overflow es detecta amb una sola porta lògica
afegida al darrer sumador: $v = c_{n-1} \oplus c_n$.
:::

### Detecció de l'overflow per programari {#sec-overflow-programari}

Donada la suma $s = a + b$ d'enters de 32 bits, la condició d'overflow és:

$$v = \overline{(A_{31} \oplus B_{31})} \land (A_{31} \oplus S_{31})$$ {#eq-overflow-software}

és a dir, overflow si els operands **tenen el mateix signe** i el resultat **té signe diferent**.

::: {#tip-overflow-programari .callout-tip}
## Càlcul de la condició d'overflow per programari
...
```{.s filename="RV32I"}
        ...
        and     t3, t3, t4      # condició d'overflow als 32 bits
        srli    t3, t3, 31      # desplaça bit 31 a la posició 0
                                # t3 = 1 si overflow, 0 si no
```
:::

::: {#imp-overflow-riscv .callout-important}
A **RISC-V**, totes les instruccions d'aritmètica entera **ignoren l'overflow**: el resultat es trunca silenciosament als 32 bits de menys pes, sense excepció ni indicador. La detecció d'overflow, si cal, és responsabilitat del programari (@sec-overflow-programari).
:::
...
![Circuit multiplicador seqüencial.](figures/TODO.png){#fig-TODO1 width="60%"}

El pseudocodi de l'algorisme és:
...
El producte de dos enters de 32 bits pot tenir fins a 64 bits de resultat exacte. `mul` retorna només els **32 bits de menys pes**, que corresponen al resultat truncat; si el resultat exacte no és representable amb 32 bits, es produeix **overflow** i el valor retornat per `mul` no és correcte.

Per obtenir el resultat exacte de 64 bits cal combinar `mul` amb `mulh` (enters amb signe) o `mulhu` (naturals sense signe), que retornen els **32 bits de més pes**. A partir d'aquests es pot detectar l'overflow:

* En **naturals** (`mulhu`): hi ha overflow si `mulhu` retorna un valor diferent de zero.
* En **enters** (`mulh`): hi ha overflow si el resultat de `mulh` no és l'extensió de signe del resultat de `mul`.

::: {#tip-overflow-multiplicacio .callout-tip}
## Detecció d'overflow en la multiplicació entera
...
En RV32IM, per obtenir el producte exacte de 64 bits i detectar overflow:

```{.s filename="RV32I"}
        # a, b als registres t0, t1
        mul     t2, t0, t1      # t2 <- bits [31:0] del producte
        mulh    t3, t0, t1      # t3 <- bits [63:32] del producte (enters)
        # overflow si t3 != extensió de signe de t2
...
* **Overflow enters**: l'únic cas possible és $x = -2^{31}$, $y = -1$, ja que el resultat exacte $2^{31}$ no és representable en Ca2 amb 32 bits. Per convenció, `div` retorna $-2^{31}$ i `rem` retorna 0.
