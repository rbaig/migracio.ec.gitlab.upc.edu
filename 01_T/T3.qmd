...
<!--
    TODO Roger `git show a288aaf`
        1. revisió de paràgraf cos del text, #cau-cas-extensio-zero i #cau-immediats-logics-extensio-signe
        2. reparar link @nte-pseudoinstruccions-salt-condicional-zero
-->

<!-- TODO @Adria: validar si he fet bé (Roger) la integració del teu patch -->
...
Cadascuna té també una variant amb immediat (`andi`, `ori`, `xori`), on l'immediat de 12 bits s'estén a 32 bits amb **extensió de signe**.
:::

::: {#cau-immediats-logics-extensio-signe .callout-caution}
Els immediats de `andi`, `ori` i `xori` no permeten construir màscares arbitràries de 32 bits. Si el bit 11 val `1`, els bits 31..12 queden a `1` per extensió de signe. Per a màscares més grans, cal carregar la màscara en un registre amb `li` i usar `and`/`or`/`xor`.
:::
...
En les taules d'instruccions, `offset13` no vol dir que la instrucció contingui 13 bits independents de desplaçament. En el format B es codifiquen 12 bits del desplaçament i el bit 0 és implícit perquè les destinacions de salt estan alineades, com a mínim, a 2 bytes. En executar la instrucció, aquest bit implícit val `0` i el desplaçament de 13 bits resultant s'estén de signe; per això l'operació s'escriu com `PC + SignExt(offset13)`. El rang efectiu és de **±4 KiB** respecte a la instrucció de salt.

A **RV32I** base (sense l'extensió C, cas d'**EC**), totes les instruccions tenen 32 bits i estan alineades a 4 bytes. Tot i això, el format B manté la granularitat de 2 bytes per compatibilitat amb l'extensió **C** (*Compressed*).
...
Com en el format B, `offset21` no vol dir que el format J emmagatzemi 21 bits independents de desplaçament. La instrucció codifica 20 bits del desplaçament i el bit 0 és implícit; en executar-la, aquest bit val `0` i el desplaçament de 21 bits resultant s'estén de signe. El rang efectiu és de **±1 MiB** respecte a la instrucció de salt.
...
- `ra` és **caller-saved**, però una subrutina multinivell l'ha de desar abans de fer una crida i restaurar-lo abans de retornar, ja que `jal` sobreescriu aquest registre en fer la crida a la subrutina filla.
:::
