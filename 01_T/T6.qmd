...
Es pot reduir el temps d'execució reduint el retard $CPI_{i}$ de cada tipus d'instrucció, però això requereix millorar el disseny de la microarquitectura. També podem reduir el nombre d'instruccions $n_{i}$ d'aquells tipus d'instruccions més lentes (amb major $CPI_{i}$) encara que sigui a costa d'augmentar el nombre de les més ràpides, si la suma és inferior, millorant l'habilitat del compilador.
...
::: {#tbl-exemple-comparacio-versions-progs tbl-colwidths="[18,10,18,18,18,18]" style="width: 70%;"}
...
::: {.light-content}
![](../figures/T6_tc_tc_prima_light.svg)
:::
::: {.dark-content}
![](../figures/T6_tc_tc_prima_dark.svg)
:::
:::
...
no canvia el nombre de cicles, però la tasca B no podrà escriure en el primer cicle i necessitarà un cicle més: necessitarà el doble de cicles que abans. La reducció de tc només suposarà una millora de rendiment si la influència del cas A en el temps d’execució total és major que la de B.
::::
...
**Pregunta**: Suposem un computador A, amb $f_{A}=2\text{GHz}$, que executa un programa en $t_{exe}=10\text{s}$. Amb certes millores al circuit, s'executa en 6s, però augmenta el nombre de cicles en un factor 1,2. Quina freqüència de rellotge té el nou disseny?
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](../figures/T6_amdahl_light.svg)
:::
::: {.dark-content}
![](../figures/T6_amdahl_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](../figures/T6_amdahl_light.svg)
:::
Temps total del programa original dividit en una part optimitzada (fracció $P_{x}$) i una part no optimitzada ($1 - P_{x}$), comparat amb el programa millorat que mostra un temps reduït $t_{1}$ on la part optimitzada es divideix pel guany $s_{x}$.
::::
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](../figures/T6_not_cmos_light.svg)
:::
::: {.dark-content}
![](../figures/T6_not_cmos_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](../figures/T6_not_cmos_light.svg)
...
El transistor *p-channel metal–oxide–semiconductor* (PMOS) condueix quan la tensió d'entrada baixa d'un cert llindar (*threshold*) $V_{t}$, i roman tallat (circuit-obert) si la tensió és superior. A la inversa, un transistor *nmos* condueix quan la tensió d’entrada puja per sobre de cert llindar $V_{t}$, i resta obert si la tensió és inferior. Aquests llindars determinen els nivells lògics 1 i 0.
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](../figures/T6_not_1_0_light.svg)
:::
::: {.dark-content}
![](../figures/T6_not_1_0_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](../figures/T6_not_1_0_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](../figures/T6_not_0_1_light.svg)
:::
::: {.dark-content}
![](../figures/T6_not_0_1_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](../figures/T6_not_0_1_light.svg)
...
- **Fuita subllindar** (*subthreshold leakage*): corrent que circula entre drenador i font d'un transistor amb una tensió de porta per sota del llindar $V_{t}$. Creix com més petit és aquest llindar, i els transistors miniaturitzats el redueixen per poder funcionar a tensions $Vcc$ més baixes.
...
Donada la tensió d'alimentació $Vcc$ i el corrent de fuita agregat de tot el circuit $I_{fuita}$, la potència estàtica és el seu producte:

$$P_{s} = Vcc \cdot I_{fuita}$$ {#eq-potencia-estatica}
