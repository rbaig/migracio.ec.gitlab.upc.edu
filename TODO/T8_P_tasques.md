...
- 🔵⏳ **C1 — Bit de presència: P (PE/PS) vs. V (teoria).** La teoria adopta
  **V** (RISC-V Sv32, Harris & Harris) i ho documenta a `#wrn-mv-notacio-v`;
  ...
  - (b) Mantenir P als problemes (familiaritat amb exàmens antics) — contradiu
    el criteri establert i la coherència Teoria↔PS.
- 🔵⏳ **C2 — `sol-p8-tlb-estat` apartat b (16 KiB sobre estat fix).** Errors
  confirmats per simulació: «El TLB de **32** entrades» (és de **4**; el de 32
  és el d'`exr-p8-mv-cache-tlb`) i «menys que les **4** originals» (la Llista 2
  en tenia **3**, i amb pàgines de 16 KiB en continuen sortint **3**: la
  conclusió «menys fallades» és falsa per a la Llista 2). A més, reutilitzar
  l'estat inicial del TLB amb una altra mida de pàgina és exactament el cas que
  `07_contrib §T8` mana reformular.
  - **(a) Recomanada:** solució qualitativa: la Llista 1 passa de 5 a 2 pàgines
    diferents i la Llista 2 de 5 a 3 → menys fallades potencials (fredes),
    matisant que el nombre exacte no és comparable perquè l'estat inicial deixa
    de ser coherent amb la nova mida.
  - (b) Reformular l'enunciat b) perquè parteixi de TLB inicialment buit →
    resultat quantitatiu net (Llista 1: 5→2 fallades; Llista 2: 5→3).
  - (c) Mantenir l'estat fix (assumint vàlides les entrades en la nova
    numeració) i corregir només els nombres (L1: 3→2; L2: 3→3, «es manté») —
    correcte però confús i contrari al criteri codificat.
- 🔵⏳ **C3 — `exr-p8-mv-matriu-lru`: `int` de 16 bits.** «`int MAT[32][256]`»
  + «els enters es codifiquen en 16 bits» contradiu l'`int` de 4 bytes del curs
  (T2). Verificat: amb 2 B/element la numerologia funciona (matriu de 16 KiB,
  2 files/pàgina; respostes: b) 16 fallades, d) 512).
  - **(a) Recomanada:** declarar `short MAT[32][256];` i «els `short` ocupen
    2 bytes» — cap altre canvi.
  - (b) Mantenir `int` amb nota «en aquest sistema experimental `int` ocupa
    2 bytes» (legal en C, però xoca amb T2).
  - (c) Passar a `int` de 4 bytes i redimensionar tot l'exercici (desaconsellat:
    canvia toda la numerologia i desborda l'espai de dades).
- 🔵⏳ **C4 — `#wrn-mv-notacio-v`: atribució a Patterson & Hennessy.** El
  callout afirma que P&H usa **P** per al bit de presència de la PTE i reserva
  V per al TLB. A COD (5a ed.), P&H parla de *valid bit* també a la taula de
  pàgines; la notació **P** (*present*) és pròpia de x86, d'altres textos
  (p. ex. Stallings) i dels exàmens antics d'EC. **Verifica-ho amb el teu
  exemplar.**
  - **(a) Recomanada si no es pot verificar:** redacció neutra: «Alguns textos
    i els exàmens anteriors d'EC anomenen aquest camp **P** (*present*), com fa
    l'arquitectura x86; aquests apunts segueixen la notació **V** de RISC-V
    (Sv32) i de Harris & Harris».
  - (b) Mantenir l'atribució si el teu P&H la confirma.
- 🔵⏳ **C5 — `exr-p8-tlb-optimitzacio` c): política del TLB redissenyat.**
  «nombre mínim d'entrades» és ambigu sense fixar la política. Simulat: amb LRU
  calen **6** entrades per assolir el mínim de 12 fallades (amb 5 encara en
  surten 17); amb reemplaçament òptim (Belady) en caldrien 5.
  - **(a) Recomanada:** afegir «mantenint el reemplaçament LRU» a l'apartat
    (resposta: 12 fallades, 6 entrades).
...
- 🔵⏳ **C6 — «aliàsing» vs. «*aliasing*».** T8 usa l'anglicisme en cursiva
  ...
  - **(a) Recomanada:** «aliàsing» a tot arreu; primera aparició a T8:
    «**aliàsing** (*aliasing*)». Registrar a `07_contrib §T8`.
  - (b) Anglicisme pur «*aliasing*» a tot arreu (canviar PE/PS/criteris).
- 🔵⏳ **C7 — Veu dels exemples de teoria (transversal T7+T8).** Els
  `callout-tip` de teoria usen la 1a persona del plural («Considerem»,
  «Suposem», «Sabem») a T7 (8×) i T8 (4×); `07_contrib` prescriu la 2a del
  plural per als tips («calculeu», «suposeu que…»).
  - (a) Harmonitzar T8 (i T7) a 2a plural — fidel a `07_contrib`, però formes
    com «Sabeu que» sonen forçades en exemples narrats.
  - **(b) Recomanada:** esmenar `07_contrib` per admetre el «nosaltres»
    expositiu als exemples narrats (pràctica real consolidada de T7+T8),
    reservant la 2a plural per a instruccions directes a l'estudiant.
  - (c) Ajornar a la revisió externa.
