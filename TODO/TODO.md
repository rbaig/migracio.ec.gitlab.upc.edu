...
Repartiment: `§Decisions obertes` 10 · `§Tasques transversals` 14 ·
`§Tasques per tema` 16 · `§Tasques globals` 10.
...
- **`fwhile:` vs `fiwhile:` — el laboratori i la teoria divergeixen** (detectat 2026-09-23, en documentar la convenció d'etiquetes de bucle). La regla de `13_contrib.qmd:122` fixa el prefix de sortida **`fi-`**, que és el majoritari al corpus (18 `fifor:` + 12 `fibucle:` + 2 `fiwhile:` = 32 ocurrències). Les **4 excepcions són totes de laboratori**, i el repartiment és net:

  | Forma | On |
  | :--- | :--- |
  | `fiwhile:` | `A2.qmd:2001`, `:2051` (teoria) |
  | `fwhile:` | `L3.qmd:229`, `:543`, `:625` i `L5.qmd:267` (laboratori) |

  Substitució mecànica de 4 línies un cop es confirmi que és el que es vol; **no s'ha executat** perquè toca `L3.qmd` i `L5.qmd`, i el criteri l'ha de fixar l'usuari (podria ser deliberat que el laboratori escurci l'etiqueta). Les etiquetes són internes als fragments: cap `@ref` no hi apunta, de manera que el canvi no pot trencar cap referència creuada.
