...
- **`_start` com a primera etiqueta de `.text`** (verificat: RARS inicia l'execució a la primera instrucció de `.text`; `_start` no és cap punt d'entrada reconegut pel simulador —l'opció «start at main» només reconeix l'etiqueta `main`). A més, `_start` és el punt d'entrada i acaba amb `ecall` exit: **no ha de tenir pròleg/epíleg** (desar/restaurar `ra` i registres segurs és codi mort), tot i que sí que ha d'usar registres **segurs** per a les dades pròpies que travessen les crides. Fet a L4 (`s4_2_2.s`, `s4_3_1.s`). **Fet a L5** (revisió interna, 2026-07-21): els tres `_start` (`abs`; `descompon` sol; programa complet `descompon`+`compon`) tenien subrutines abans de `_start` i pròleg/epíleg innecessari; reordenats i corregits, amb l'anàlisi de registres segurs i bloc d'activació de la solució actualitzada corresponentment; verificat amb RARS 1.6 als tres casos. **Pendent**: L3 (`s3_4_2` amb `moda`; `s3_5_1` amb `codifica`/`g`) té subrutines abans de `_start` → mateixa fallada d'execució; revisar-ne l'ordre i el pròleg/epíleg de `_start`. L2 i L6 ja posen `_start` primer (comprovar-ne igualment l'absència de pròleg/epíleg innecessari). Un cop consolidat (falta L3), afegir la regla a `13_contrib.qmd §Convencions globals del laboratori` («`_start` ha de ser la primera etiqueta de `.text`; sense pròleg/epíleg perquè no és callee de ningú»).
