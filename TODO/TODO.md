...
- **`_start` com a primera etiqueta de `.text`** (verificat: RARS inicia l'execució a la primera instrucció de `.text`; `_start` no és cap punt d'entrada reconegut pel simulador —l'opció «start at main» només reconeix l'etiqueta `main`). A més, `_start` és el punt d'entrada i acaba amb `ecall` exit: **no ha de tenir pròleg/epíleg** (desar/restaurar `ra` i registres segurs és codi mort), tot i que sí que ha d'usar registres **segurs** per a les dades pròpies que travessen les crides. Fet a L4 (`s4_2_2.s`, `s4_3_1.s`). **Fet a L5** (revisió interna, 2026-07-21): els tres `_start` (`abs`; `descompon` sol; programa complet `descompon`+`compon`) tenien subrutines abans de `_start` i pròleg/epíleg innecessari; reordenats i corregits, amb l'anàlisi de registres segurs i bloc d'activació de la solució actualitzada corresponentment; verificat amb RARS 1.6 als tres casos. **Fet a L3** (commit `b6c8124`, 2026-09-20): `s3_4_2` i `s3_5_1` reordenats. **TANCAT** (auditoria, sessió 2, 2026-09-21): verificats programàticament els **25 blocs de `.text` amb `_start` de tot el laboratori (L1–L6): cap infracció**. La regla ja és consolidada a `13_contrib.qmd §Convencions globals del laboratori`. Nota: els 3 blocs de `.text` sense `_start` (`L2:511`, `L5:106`, `L5:239`) són fragments o fitxers de subrutina sola, i la regla —que és d'ordre intern— no els aplica.
