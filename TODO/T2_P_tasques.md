...
1. **Figura del compendi a `#nte-instruccions-tipus`** — el caption diu «(tipus R, I i S)» però `registres.toml` genera el compendi amb **R, I, S, B, U, J i R4** (B/U/J són de T3 i R4 de T5). Opcions:
   - (a) Nova entrada al TOML amb només R/I/S per a T2 (el compendi complet queda per a `11_riscv.qmd`). *Recomanada: coherència amb la linealitat pedagògica.*
   - (b) Mantenir la figura completa i corregir caption + frase de context («els formats B, U i J s'estudien al T3; R4, al T5»).
   - (c) Eliminar la figura del callout (les figures per-format ja apareixen a §Operands).
   - En tots els casos: renombrar l'ID `#fig-rv-compendi-registres` → `#fig-compendi-registres` (prefix `fig-rv-` reservat a `11_riscv.qmd`; 0 referències, canvi segur).
2. **«Sis formats bàsics»** — l'especificació parla de 4 formats nuclears (R/I/S/U) amb B/J com a variants; Patterson en diu 6. Mantenir «sis» (opció per defecte) o afegir un matís/nota. També hi ha una frase enredada a arreglar en qualsevol cas: «un **word** (32 bits, 4 bytes, a RISC-V 32 bits)» → «un **word** (a RV32I, 32 bits, 4 bytes)».
3. **Instruccions no introduïdes usades a T2** (`beq`, `bge`, `j`, `slli`, `mv`, `li` als bucles d'A2 i a molts exercicis d'E2/S2, quan salts i desplaçaments són matèria de T3). Opcions:
   - (a) Mini-callout «Avançament — instruccions de control i desplaçament» a `#sec-acces-vectors-bucles` amb semàntica d'una línia per instrucció i remissió a T3.
   - (b) Remissions puntuals «(s'estudien a T3, @nte-...)» als exemples. *Lleugera.*
   - (c) Moure els exemples de bucles a T3. *Impacte gran a E2/S2.*
4. **`#exr-p3-format-mtoc` d)** desassembla a `slt` (no introduïda fins a T3): (a) substituir el valor per una instrucció ja vista (p. ex. `sub`: `0x40B282B3` → `sub t0, t0, t1`); (b) mantenir-la amb remissió a la targeta de referència. Lligat a B.3.
...
12. **`#tip-alineacio-incorrecte`** — incoherència interna: l'adreça `0x00400020` del missatge d'error de RARS només quadra si `startup.s` (3 instruccions) precedeix el codi, però llavors el punt d'entrada hauria de ser `main` i el codi declara `_start`. Sense `startup.s`, l'adreça seria `0x00400014` (`la` = 2 instruccions). Opcions: (a) codi tal qual + adreça `0x00400014`; (b) etiqueta `main` + mantenir `0x00400020`. **Lligat al TODO(startup.s)** (bloc comentat, no tocat).
13. **`@imp-llenguatges-de-referencia` trencada** — referenciada des d'A1 (L16) i A2 (L11), però la taula no existeix enlloc. Opcions: (a) crear el callout-important amb la taula d'eines i llenguatges de referència d'EC (a A1, presumiblement: C, RV32I, RARS, GCC toolchain, versions); (b) eliminar les dues referències. Si (a), cal el contingut de la taula (o el PDF original d'on venia).
...
17. **TODO.md**: no és al mirror públic (404). Les tasques `TODO Roger unificar format taules pseudoinstruccions` (relacionada amb A.6) i `TODO(startup.s)` (relacionada amb B.12) hi poden tenir context. Passar-lo si escau.
