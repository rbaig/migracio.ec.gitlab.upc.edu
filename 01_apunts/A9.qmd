...
## Suport de maquinari: registres de control i estat {#sec-ei-csr}
...
: {tbl-colwidths="[8,12,20,50,10]" .striped}
:::
...
::: {#wrn-zicsr-immediats .callout-warning collapse=true}
...
## Zicsr ISA — Instruccions d'accés als CSR amb immediat

| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/Zicsr_instruccions_immediats.qmd >}}
: {tbl-colwidths="[5,20,35,35,5]" .striped}
:::
:::
...
: {tbl-colwidths="[10,15,75]" .striped}
:::
...
- **`mip`**: cada bit s'activa quan el dispositiu corresponent ha enviat una petició d'interrupció i la manté activa. Els bits de mode M (`MEIP`, `MTIP`, `MSIP`) són de **sols lectura** per al programari (els actualitza el maquinari).
...
## Flux de maquinari en detectar una excepció o interrupció {#sec-ei-flux-hardware}
...
- **Escriptura a `mtval`**: en algunes excepcions, el maquinari escriu informació addicional al registre `mtval` per ajudar la RSE en el tractament (vegeu @sec-ei-csr-mtval). En les interrupcions, `mtval` s'escriu a zero.
...
Un cop el maquinari ha completat les accions de @sec-ei-flux-hardware-accions, el processador comença a executar la RSE. Aquesta rutina, que forma part del SO, té les responsabilitats següents, en ordre:

1. **Salvar l'estat del programa interromput**: la RSE ha de guardar en memòria tots els registres generals (`x1`–`x31`), per poder-los restaurar en acabar. A diferència d'una subrutina normal, aquí no s'apliquen les regles de l'ABI sobre registres salvats i temporals: com que la RSE no ha estat invocada voluntàriament pel programa, cal salvar-los tots sense excepció.
...
### Convenció de crides al sistema a RARS {#sec-ei-ecall-abi-rars}
...
Els dos números tenen orígens diferents, i això explica la duplicitat. El catàleg de RARS [@rars_v16] combina dues famílies de serveis: els heretats de **MARS**, el simulador de MIPS del qual RARS descendeix, i els compatibles amb les eines de Linux. El **10** prové de la primera. El **93**, en canvi, és el número real de la crida `exit` a **Linux/RISC-V** (`asm-generic/unistd.h`), el mateix origen que el `63` (`read`), el `64` (`write`) o el `214` (`brk`) de @sec-ei-ecall-abi-linux. RARS conserva els dos noms heretats de MARS i adopta per a `exit2` el número de Linux; per això el mateix servei apareix com a `exit2` a la taula de RARS i com a `exit` a la de Linux (@sec-ei-ecall-abi-linux).
...
: RARS: codis de servei més rellevants. {#tbl-syscalls-rars .striped}
:::
...
**A EC**, però, la sortida dels programes es fa sempre amb `exit2` (codi 93), que permet especificar el codi de sortida a `a0` (@nte-programa-esquelet):

```{.s filename="RV32I"}
        li      a0, 0               # codi de sortida: 0
        li      a7, 93              # codi de exit2
        ecall                       # no retorna
```
:::

### Convenció ABI de Linux {#sec-ei-ecall-abi-linux}
...
::: {#wrn-ecall-exemples-linux .callout-warning collapse=true}
## Aprofundiment: exemples de crides al sistema Linux/RISC-V (RV32)
...
| 48 | `faccessat` | `a0`=dirfd, `a1`=path, `a2`=mode | `a0`=0 o error |
...
| 62 | `llseek` | `a0`=fd, `a1`=offset_hi, `a2`=offset_lo, `a3`=resultat, `a4`=whence | `a0`=0 o error |
...
| 403 | `clock_gettime64` | `a0`=clkid, `a1`=tp | `a0`=0 o error |
...
3. **Acceptació**: el maquinari executa les accions de @sec-ei-flux-hardware-accions: guarda el PC a `mepc`, escriu `mcause` amb bit 31=1 i el codi de causa, desactiva `MIE` i salta a `mtvec`.
...
- **Permís global `MIE`** (bit 3 de `mstatus`): habilita o deshabilita totes les interrupcions alhora. El maquinari el posa a 0 automàticament en acceptar qualsevol excepció o interrupció, i `mret` el restaura al valor de `MPIE`.
...
- **`ASID`** (bits 30:22, *Address Space Identifier*): identificador de l'espai d'adreces, que permet al TLB distingir traduccions de processos diferents sense haver-lo de buidar en cada canvi de context.
