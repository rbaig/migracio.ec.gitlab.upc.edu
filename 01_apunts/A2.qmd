...
- **Sistema**: `ecall`, `ebreak`, `fence` (`fence.tso` és una variant codificada dins `fence`)

A **EC**, s'estudien totes excepte les de sistema, de les quals només es tracta `ecall` (@sec-ei-ecall).
:::

:::: {#cau-prefixos-binaris .callout-caution}
::: {#tbl-prefixos-binaris tbl-colwidths="[20,15,30,35]" style="width: 90%; margin: auto;"}
| Prefix | Símbol | Valor | Origen |
| :--- | :---: | :--- | :--- |
| Kilo / Kibi | `KB` / `KiB` | $10^3 = 1000$ / $2^{10} = 1024$ bytes | SI (decimal) / IEC (binari) |
| Mega / Mebi | `MB` / `MiB` | $10^6$ / $2^{20} = 1\,048\,576$ bytes | SI (decimal) / IEC (binari) |
| Giga / Gibi | `GB` / `GiB` | $10^9$ / $2^{30}$ bytes | SI (decimal) / IEC (binari) |
| Tera / Tebi | `TB` / `TiB` | $10^{12}$ / $2^{40}$ bytes | SI (decimal) / IEC (binari) |
: Prefixos de mida: amb «i» (binaris, potències de 2) vs. sense «i» (decimals, potències de 10)
:::
...
: GCC, paràmetres per la generació de codi d'assemblador sense optimitzacions {#tbl-gcc-rvimf tbl-colwidths="[20,7,73]" style="width: 90%;"}
...
L'arquitectura **RV32I** defineix quatre formats nuclears d'instrucció [@riscv_rv32i]: **R** (Register), **I** (Immediate), **S** (Store) i **U** (Upper). Dos formats addicionals, **B** (Branch) i **J** (Jump), són variants de S i U respectivament: reorganitzen els mateixos bits d'immediat per codificar desplaçaments sempre parells (el bit 0 és implícit), i s'introdueixen a [Tema 3](#sec-tema-traduccio) amb les instruccions de salt (@nte-format-b, @nte-format-j). @nte-instruccions-tipus mostra els **camps de bits** dels tres formats que s'usen en aquest tema (R, I, S); el format U s'introdueix més endavant en aquest mateix tema (@nte-instruccio-lui). El [compendi de referència tècnica](#sec-reftecnica-rv32i-format) (@nte-rv-instruccions-formats-detall) recull tots els formats en una única figura, a mesura que es vagin presentant al llarg del curs.

::: {#nte-instruccions-tipus .callout-note}
## RV32I ISA — Tipus de format de les instruccions

::: {#fig-compendi-registres}
...
: {tbl-colwidths="[9,26,9,9,9,9,9,8,12]"}
...
: {tbl-colwidths="[18,18,27,37]" style="width: 60%; margin: auto;"}

**L'execució comença a la primera instrucció del segment `.text`**, és a dir, a l'adreça `0x00400000` de la taula: RARS hi posa el PC en començar i no cerca cap etiqueta d'entrada. Per això, als programes d'aquests apunts, el punt d'entrada **no porta etiqueta** i el `.text` del programa principal va **abans** de qualsevol subrutina. El motiu de fons —que a EC es programa sense sistema operatiu— és a @nte-punt-entrada-etiqueta.

**Forma llarga de la directiva.** L'ABI ELF admet també `.section .data` i `.section .text`, que és la forma que genera `gcc` i la que trobareu en codi real. **A EC s'usa sempre la forma curta** (`.data`, `.text`): RARS no accepta la forma llarga per a aquests dos segments —dona l'error `.section must be followed by a section name`— i, en les variants que sí que assembla, la directiva **no commuta de segment**, de manera que les dades o el codi que la segueixen no van on sembla. És una limitació del simulador, no una diferència de l'arquitectura.
...
::: {tbl-colwidths="[18,9,73]" style="width: 90%; margin: auto;"}
...
::: {#nte-rars-noms-reservats .callout-note}
## RARS — Noms reservats

A **RARS**, un nom que trieu vosaltres —una etiqueta o una constant simbòlica— **no pot coincidir amb cap nom que l'assemblador ja conegui**: cap mnemònic (`add`, `b`, `j`, `li`, `mv`, `ret`, `nop`…) ni cap nom de registre (`t0`, `a0`, `zero`…). **El missatge d'error no us ho diu**, i és per això que costa de trobar.

Els dos missatges amb què hi arribareu:

| On | Nom que hi poseu | Missatge |
| :--- | :--- | :--- |
| `.eqv` | mnemònic o registre | `Malformed .eqv directive` |
| Etiqueta a `.text` | registre en minúscules | `"t0" is not a recognized operator` |

Exemple concret: si voleu una constant per a la mida d'un bloc i escriviu

```{.s filename="⚠️codi_erroni__nom_reservat.s⚠️"}
.eqv    B, 16           # ✗ Malformed .eqv directive
```

falla perquè `B` és el salt incondicional `b`, i **els mnemònics no distingeixen majúscules de minúscules**. La solució és un nom més descriptiu, que a més es llegeix millor:

```{.s filename="RV32I"}
.eqv    BLK, 16         # ✓
```

Dos detalls que fan el diagnòstic més difícil:

- **Les dues cares no es comporten igual.** Una *etiqueta* sí que pot dir-se `add` o `b` (els mnemònics només són reservats per a `.eqv`); en canvi un nom de *registre* en minúscules com a etiqueta només falla al segment `.text`.
- **Al segment `.data`, una etiqueta anomenada `t0` assembla**, però és inservible: l'error apareix més tard, on la useu (`la t1, t0` → `"t0": operand is of incorrect type`).

En majúscules (`T0`, `ZERO`) no hi ha xoc, perquè els noms de registre sí que distingeixen majúscules; però **no és una solució recomanable**: un nom com `T0` confon el lector. Trieu noms descriptius i el problema no apareix.
:::
...
A **RARS**, les constants simbòliques es declaren mitjançant la directiva `.eqv` (`.equ` / `.set` en GNU assembly). El nom que se'ls dona no pot coincidir amb cap mnemònic ni cap nom de registre (@nte-rars-noms-reservats).
...
::: {#nte-rars-operands-literals .callout-note}
## RARS — Operands sense expressions aritmètiques
...
.data
        .align  2
mat:    .space  120      # int mat[NF][NC]: NF*NC*4 = 120 bytes
```
:::

::: {#imp-ec-operands-expressions .callout-important}
A **EC**, a la teoria i problemes i als exàmens, si no s'indica explícitament el contrari, **s'admeten expressions aritmètiques als operands** (`.space NF*NC*4`, `li t0, NC*4`, `la t0, vec+8`): fan visible l'estructura del càlcul, que és el que s'hi vol ensenyar. Un `NC*4` diu què és el valor; un `24`, no.

Al **laboratori**, en canvi, cal escriure-hi sempre el **literal ja calculat**, amb l'expressió aritmètica al comentari (@nte-rars-operands-literals). El cas de `la rd, etiqueta+desplaçament` té una substitució pròpia —la parella `la` + `addi`— i es detalla a @imp-ec-la-offset.
:::
...
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32I_instruccions_aritmetiques.qmd >}}
...
::: {#cau-rv32i-ignora-sobreeiximent .callout-caution}
A **RV32I**, les operacions de suma i resta **ignoren** el sobreeiximent (*overflow*): el resultat és simplement els 32 bits de menys pes de l'operació, independentment de si el resultat és correcte o no en el rang dels enters representables.
:::
...
| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32I_instruccions_lui.qmd >}}
: {tbl-colwidths="[5,20,35,35,5]" .striped}
...
| :--- | :--- | :--- |
{{< include ../21_riscv/RV32I_pseudo_mv.qmd >}}
...
Cas particular d'@imp-ec-operands-expressions. A **EC**, a la teoria i problemes i als exàmens, si no s'indica explícitament el contrari, s'admet la sintaxi més llegible següent:
...
**Exemple**: macro que inicialitza un element d'un vector global d'enters a zero, donat l'índex. L'escalat de l'índex es fa amb un desplaçament de bits (@sec-desplacaments-de-bits):

```{.s filename="RV32I"}
.macro zero_elem (%base, %idx)
        slli    t6, %idx, 2         # offset = idx * 4 (desplaçament de bits, T3)
...
El `.text` no porta cap etiqueta d'entrada: l'execució comença a la seva **primera instrucció** (@sec-programa-segments).
:::
...
::: {tbl-colwidths="[33,13,24,30]" style="width: 80%; margin: auto;"}
...
- Les adreces del programa **poden ser virtuals**: en un sistema amb sistema operatiu, la **MMU** (***Memory Management Unit***) les tradueix a adreces físiques (@sec-mv-espais, [Tema 8](#sec-tema-mv)). A **RARS** no hi ha traducció: les adreces del programa **són** directament les adreces físiques.
...
::: {#tbl-LSb-LSB tbl-colwidths="[42,42,16]" style="width: 60%;"}
...
::: {#fig-endianness-regla-pi}
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T2_endianness_regla_pi__original_light.svg){width="40%"}
:::
::: {.dark-content}
![](/auto_figs/T2_endianness_regla_pi__original_dark.svg){width="40%"}
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T2_endianness_regla_pi__original_light.svg){width="50%"}
:::
Regla mnemotècnica de la lletra grega pi (Π) per recordar l'ordenació dels bytes Little-endian i Big-endian.
:::
::::
...
<!-- TODO: Verificar que la informació de la taula és correcta i coincideix amb l'ABI `ilp32` **i que no hi ha col·lisió amb l'ABI RV de Bloc d'Activació alineació a 16** -->
::: {#cau-memoria-restriccions-alineacio .callout-caution}
A **EC**, es segueixen les restriccions definides per l'**ABI** `ilp32` de RISC-V.
:::

::: {#nte-restriccions-alineacio .callout-note}
...
Error in /.../codi.s line 14: Runtime exception at 0x00400014: Load address not aligned on halfword boundary 0x10010005
```
:::
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T2_instruccio_tipus_R__registre_light.svg )
:::
::: {.dark-content}
![](/auto_figs/T2_instruccio_tipus_R__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T2_instruccio_tipus_R__registre_light.svg )
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T2_instruccio_tipus_I__registre_light.svg )
:::
::: {.dark-content}
![](/auto_figs/T2_instruccio_tipus_I__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T2_instruccio_tipus_I__registre_light.svg )
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T2_instruccio_tipus_S__registre_light.svg )
:::
::: {.dark-content}
![](/auto_figs/T2_instruccio_tipus_S__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T2_instruccio_tipus_S__registre_light.svg )
...
: {tbl-colwidths="[6,24,47,17,6]" .striped}
:::
...
#### Accés a enters de mitja paraula o byte: `lh`, `lb`, `sh`, `sb` {#sec-acces-mig-paraula-byte}
...
## Operands en mode base + desplaçament, accés a enters de mitja paraula o byte
...
| `lh t1, 0(t2)` | Càrrega de la mitja paraula `0x2211` | `t1 = 0x00002211` |
| `lh t1, 2(t2)` | Càrrega de la mitja paraula `0xDDCC` | `t1 = 0xFFFFDDCC` |
...
## Operands en mode base + desplaçament, càrrega de naturals de mitja paraula o byte
...
2. **Corrupció (silenciosa) de dades**: Si l'adreça calculada amb l'aritmètica de punters encara cau dins de la memòria que el programa té assignada (per exemple, on hi ha una altra variable veïna al segment de dades, @sec-memoria-t2), **el programa continuarà funcionant**, però **accedint a dades indeterminades** sense cap indicació d'error.
...
## Directives — Reserva de memòria: `.space`, `.align`

| Directiva | Operands | Descripció |
| :--- | :--- | :--- |
| `.align` | `n` | Alinea el següent element a la frontera de $2^n$ bytes |
...
$$@\texttt{vec[i]} = @\texttt{vec[0]} + i \cdot T$$ {#eq-acces-aleatori-vector}
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T2_acces_vector__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T2_acces_vector__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T2_acces_vector__original_light.svg)
...
- L'**accés aleatori** (@sec-vectors-acces-aleatori; per a matrius, @sec-matrius-acces-aleatori), que consisteix en calcular l'adreça de cada accés a cada iteració (@eq-acces-aleatori-vector, @eq-acces-aleatori-matriu).
- L'**accés seqüencial** (@sec-acces-sequencial-exemple; optimització definida a @sec-opt-acces-sequencial), que manté un punter que avança una quantitat fixa (*stride*) a cada iteració.
...
[^comram]: És el mateix cas d'ús que en la sigla RAM (*Random Access Memory*), que no significa memòria a l'atzar, sinó memòria que permet accés directe a qualsevol adreça.

::: {#cau-avancament-instruccions-bucles .callout-caution}
Els exemples de bucle d'aquesta secció usen instruccions de salt condicional (`beq`) i incondicional (`j`), i de desplaçament de bits (`slli`), que es presenten formalment al [Tema 3](#sec-tema-traduccio) (@sec-desplacaments-de-bits, @sec-salts). De moment n'hi ha prou d'entendre-les intuïtivament pel comentari: `beq rs1, rs2, etq` salta a `etq` si `rs1 = rs2`; `j etq` salta sempre a `etq`; `slli rd, rs1, n` desplaça `rs1` `n` bits cap a l'esquerra (equivalent a multiplicar per $2^n$).
:::

::: {#cau-void-main .callout-caution}
A partir d'aquí, els programes en C d'aquests apunts s'escriuen amb `void main()`: són **notació** del programa que traduireu a mà a assemblador, no codi que es compili, i no hi ha cap SO que en reculli el valor de retorn (@imp-directe-sobre-processador). **En un programa de C normal, compilat per a un sistema operatiu, cal `int main`.**
:::
