...
L'ABI ELF admet també una **forma llarga** de la directiva de segment, `.section .data` i `.section .text`, que és la que genera `gcc` i la que trobareu en codi real. **A EC s'usa sempre la forma curta** (`.data`, `.text`): RARS no accepta la forma llarga per a aquests dos segments —dona l'error `.section must be followed by a section name`— i, en les variants que sí que assembla, la directiva no commuta de segment. És una limitació del simulador, no una diferència de l'arquitectura.
::::
:::::
