...
Les instruccions `sll` i `srl` desplacen el contingut del registre `rs1` a l'esquerra o a la dreta el nombre de posicions especificat als **5 bits** de menys pes del **registre** `rs2`, omplint les posicions vacants amb **`0`s**. Les variants `slli` i `srli` fan la mateixa operació però el nombre de posicions es codifica directament com un **immediat** de **5 bits** (`shamt`, *shift amount*)[^shamt]. En tots els casos, el nombre de posicions s'interpreta com un natural.
...
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32I_instruccions_desplacament_bits_logics.qmd >}}
...
**Pregunta**: Com s'obté la seva representació en Ca1 (@sec-enters-en-ca1) i s'assigna a `t1`?
...
```{.s filename="RV32I"}
        andi    t0, t1, -2048   # t0 = t1 & 0xFFFFF800  (no t1 & 0x00000800!)
```
...
| :--- | :--- | :--- |
{{< include ../21_riscv/RV32I_pseudo_not.qmd >}}
...
Selecció dels 16 bits de menys pes de `rs1`:
...
Posada dels 16 bits de menys pes de `t0` a `1`:
...
Les instruccions de salt condicional són de format **Tipus-B** (@nte-format-b), variant del format **Tipus-S** ja presentat a [Tema 2](#sec-format-instruccions) (@nte-instruccions-Tipus-S). El desplaçament codificat és de **13 bits amb signe**, però el bit 0 és sempre implícit (val `0`) perquè les destinacions de salt estan alineades a, com a mínim, 2 bytes. D'aquesta manera, la instrucció emmagatzema 12 bits i obté un rang efectiu de **±4 KiB** (@cau-prefixos-binaris) respecte al PC. A **RV32I** base (cas d'**EC**), les instruccions estan alineades a 4 bytes; el format B manté la granularitat de 2 bytes per compatibilitat amb l'extensió **C** (*Compressed*).
...
::: {.light-content}
![](/auto_figs/T3_instruccio_tipus_B__registre_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_instruccio_tipus_B__registre_dark.svg)
:::
:::
...
La instrucció `jal` és de format **Tipus-J** (@nte-format-j), variant del format **Tipus-U** (@nte-format-u), presentat més avall en aquest mateix tema a partir de la pseudoinstrucció `la` (i introduït a [Tema 2](#sec-format-instruccions) amb `lui`, @nte-instruccio-lui). Com en el format B (@nte-format-b), el bit 0 de l'immediat és implícit (val `0`), de manera que la instrucció codifica 20 bits i obté un rang efectiu de **±1 MiB** respecte al PC.
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_instruccio_tipus_J__registre_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_instruccio_tipus_J__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_instruccio_tipus_J__registre_light.svg)
...
| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32I_instruccions_salt_incondicional.qmd >}}
: {tbl-colwidths="[5,20,35,35,5]"}
:::
...
| :--- | :--- | :--- |
{{< include ../21_riscv/RV32I_pseudo_j.qmd >}}
:::
...
| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32I_instruccions_salt_incondicional_indirecte.qmd >}}
: {tbl-colwidths="[5,20,35,35,5]"}
:::
...
| Pseudoinstrucció | Expansió | Ús |
| :--- | :--- | :--- |
{{< include ../21_riscv/RV32I_pseudo_salt_indirecte.qmd >}}
:::
...
Ara que ja s'han presentat totes les instruccions de processament i control de flux de la base **RV32I** (@nte-riscv-rv32i), tret de `ecall` (@sec-ei-ecall) —les instruccions de sistema, com `ecall` i `ebreak` (presentada a Tema 9), queden fora d'aquesta classificació—, l'exercici de classificar-les segons el **mode d'adreçament**
...
- **`ecall`** no s'ha inclòs perquè no és una instrucció de processament de dades, sinó una instrucció de control que invoca una crida al sistema (@sec-ei-ecall).
...
switch_vec:
        ...
        .word   cas_3           # apunta al codi de case 3
```
...
## Avaluació de la condició al final
...
El cos del bucle s'executa ara sense cap salt incondicional: el salt condicional al final torna al principi si la condició segueix sent certa, o en cas contrari cau a `fibucle`. Això estalvia un salt per iteració. Aquesta transformació es formalitza més endavant com a Optimització #2 (@sec-optimitzacions-bucle a Tema 4).
...
## Traducció ingènua del bucle `for`
...
## Eliminació de la variable d'inducció

Una **variable d'inducció** és una variable que s'incrementa o decrementa en una quantitat constant a cada iteració d'un bucle. El compilador pot eliminar-les substituint-les per l'aritmètica de punters directament. Aquesta transformació es formalitza més endavant com a Optimització #3 (@sec-optimitzacions-bucle a Tema 4).
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_mapa_memoria__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_mapa_memoria__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_mapa_memoria__original_light.svg)
...
Les dues instruccions en què s'expandeix `la` pertanyen al format **Tipus-U** (@nte-format-u): `lui` (ja presentada a [Tema 2](#sec-format-instruccions), @nte-instruccio-lui) i `auipc`.
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_instruccio_tipus_U__registre_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_instruccio_tipus_U__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_instruccio_tipus_U__registre_light.svg)
...
`funcA` és una subrutina **multinivell**: crida `funcB` i després continua executant-se fins al seu propi `ret`, de manera que haurà de preservar `ra` abans de la crida (@sec-multinivell-pila). S'omet aquí per centrar l'exemple en el pas de paràmetres.
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_func_uninivell_pila__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_func_uninivell_pila__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_func_uninivell_pila__original_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_ba_general__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_ba_general__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_ba_general__original_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_ba_func__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_ba_func__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_ba_func__original_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_pila_crides_aniuades__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_pila_crides_aniuades__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_pila_crides_aniuades__original_light.svg)
...
:::
::: {.dark-content}
![](/auto_figs/T3_deps_multi__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_deps_multi__original_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_ba_multi__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_ba_multi__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_ba_multi__original_light.svg)
...
:::
::: {.dark-content}
![](/auto_figs/T3_deps_exemple__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_deps_exemple__original_light.svg)
...
:::
::: {.dark-content}
![](/auto_figs/T3_ba_exemple__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_ba_exemple__original_light.svg)
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_compilacio_separada__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_compilacio_separada__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_compilacio_separada__original_light.svg)
...
:::: {#nte-punt-entrada-etiqueta .callout-note}
## El punt d'entrada del programa i l'etiqueta `_start`

En **GNU/Linux**, el punt d'entrada d'un executable es marca amb l'etiqueta **`_start`**, declarada global amb `.globl _start`. El nom no és decoratiu: l'**enllaçador** l'ha de localitzar per escriure'l a la capçalera de l'executable, i el **carregador** del sistema operatiu hi transfereix el control en engegar el procés. Hi ha, doncs, dos agents que llegeixen aquell nom, i per això el símbol ha de ser global i s'ha d'anomenar exactament així.

A **EC** es programa directament sobre el processador (@imp-directe-sobre-processador), en un **entorn autònom** (*bare-metal*, @sec-entorn-autonom-bare-metal): no hi ha ni enllaçador ni carregador, i **cap agent no llegeix el nom del punt d'entrada**. L'execució comença sempre a la **primera instrucció del segment `.text`**, de manera que als programes d'aquests apunts el punt d'entrada **no porta etiqueta**.

No és una peculiaritat de l'assignatura, sinó la diferència entre un entorn **allotjat** (*hosted*) i un **autònom**: en un entorn autònom, la forma i el nom del punt d'entrada són **definits per la implementació**. És el mateix motiu pel qual el C d'aquests apunts declara `void main()` i no `int main` (@cau-void-main): una etiqueta que ningú no resol seria tan fictícia com un `return` que ningú no recull.
::::
...
El mecanisme descrit en aquesta secció és el d'un **sistema real amb SO**. A **EC** no hi ha ni SO ni carregador (@imp-directe-sobre-processador): els programes no reben `argc` ni `argv` i no tornen enlloc en acabar. El punt d'entrada és la **primera instrucció del segment `.text`**, sense cap etiqueta que l'anomeni (@nte-punt-entrada-etiqueta), i la sortida es fa amb `li a7, 93` + `ecall` (@nte-programa-esquelet).

El `void main()` dels programes en C d'aquests apunts no contradiu res del que s'acaba de dir: és **notació** del programa que traduireu a mà (@cau-void-main), no un punt d'entrada que cap rutina de *startup* no arriba a cridar.
:::
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T3_flux_gcc_complet__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T3_flux_gcc_complet__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T3_flux_gcc_complet__original_light.svg)
