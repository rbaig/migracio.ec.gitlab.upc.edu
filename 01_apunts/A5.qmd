...
on l'**exponent** $n$ és un enter i la **mantissa** $m$ satisfà $1 \leq |m| < 10$, amb almenys un dígit fraccionari. Aquesta restricció s'anomena **condició de normalització** i garanteix una representació única per a cada nombre.

La notació científica normalitzada presenta dos avantatges respecte de la representació decimal ordinària. D'una banda, és més compacta: $2{,}34 \times 10^8$ és més llegible que $234\,000\,000$. D'altra banda, fa explícit el nombre de dígits significatius: $2{,}34 \times 10^8$ indica clarament tres dígits significatius, mentre que $234\,000\,000$ és ambigu (poden ser tres o bé pot ser que el nombre sigui exacte fins a les unitats). El terme **coma flotant** fa referència precisament a aquest mecanisme: la coma «es desplaça» cap a l'esquerra o cap a la dreta en funció del valor de l'exponent.
...
on l'exponent $e$ és un enter i $F$ és la part fraccionària de la mantissa.
...
::: {.light-content}
![](/auto_figs/T5_ieee754_format_registre__registre_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T5_ieee754_format_registre__registre_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T5_ieee754_format_registre__registre_light.svg)
:::
Disposició dels camps S, E i F en el format IEEE 754 de simple precisió (32 bits).
:::
...
Els bits a descartar comencen per `10001...`; com que el primer bit descartat és `1` i n'hi ha d'altres a la dreta, cal arrodonir **al valor següent** (sumar 1 al bit de menys pes conservat). El criteri general d'arrodoniment es formalitza a @sec-arrodoniment; en aquest cas:
...
Donat un nombre fix de bits, augmentar la precisió (més bits de fracció) redueix el rang (menys bits d'exponent) i viceversa. El format de simple precisió (8 bits d'exponent i 23 de fracció) és el compromís adoptat per l'estàndard IEEE 754 per a aplicacions de propòsit general.
:::

<!-- TODO: PM: la paraula denormals de la figura a baix no s'aprecia del tot bé -->
...
L'estàndard IEEE 754 reserva dos valors del camp d'exponent per a codificacions especials: $E = 0000\,0000_2$ (tot zeros) i $E = 1111\,1111_2$ (tot uns). Per tant, el **rang efectiu** de l'exponent queda reduït a $[-126,\ +127]$ en simple precisió.

<!-- TODO: PM: Error a la caption de la taula -->
...
Com s'ha vist a la @sec-error-representacio, quan $|v| < 2^{-126}$ l'error relatiu pot arribar al 100% si s'arrodoneix a zero. Els nombres **denormals** (*subnormals*) cobreixen l'interval $(0,\ 2^{-126})$ amb nombres de la forma:

$$v = \pm 0{,}F \times 2^{-126} \qquad (F \neq 0)$$

A diferència dels nombres normalitzats, **no hi ha bit ocult per als denormals**: la part entera de la mantissa és $0$ (en comptes de $1$). L'exponent implícit dels denormals és sempre $e = -126$; és a dir, no s'obté aplicant la fórmula $e = E_u - 127$ (que amb $E_u = 0$ donaria $-127$), sinó que és, per definició, el mateix exponent que el del normal més petit ($2^{-126}$).
...
L'**error relatiu creix de forma gradual** a mesura que $v$ s'acosta a zero, en lloc d'augmentar de cop com passaria sense denormals. Per aquesta raó el mecanisme es coneix també com a **subdesbordament gradual** (*gradual underflow*).
...
Per a cada exponent normalitzat $E \in [1, 254]$, la fracció $F$ pren els $2^{23}$ valors possibles $\{0, 1, \ldots, 2^{23}-1\}$. Per exemple, $F = 0$ correspon a la potència de 2 exacta $(\pm 1{,}0 \times 2^e)$, un nombre normalitzat vàlid.

Tanmateix, per als denormals ($E = 0$), el rang de $F$ és el mateix, però l'exemple $F = 0$ codifica $\pm 0$, que no és un denormal. Per tant, hi ha $2^{23} - 1$ denormals per signe, enfront dels $2^{23}$ valors per exponent dels normals. Aquesta **asimetria** és visible comparant @fig-recta-zoom-zero amb @fig-recta-global.
:::
...
Els modes d'arrodoniment a $+\infty$ i a $-\infty$ s'usen conjuntament en aritmètica d'intervals, una tècnica que acota els errors d'arrodoniment per garantir la fiabilitat dels resultats numèrics. L'estàndard inclou encara un cinquè mode pot emprat: «al més pròxim amb empat cap a la magnitud major» (RMM). RISC-V implementa els cinc modes al registre `fcsr` (@nte-fcsr).

#### Bits de guarda {#sec-bits-de-guarda}

Quan el resultat exacte d'una operació té més bits que la mantissa, els bits de guarda (els bits sobrants) determinen com s'arrodoneix. N'hi ha prou amb tres:
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T5_grs_esquema__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T5_grs_esquema__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T5_grs_esquema__original_light.svg)
:::
Esquema dels bits de guarda ($G$), arrodoniment ($R$) i *sticky* ($S$) respecte de la mantissa retinguda, amb la regla RNE.
:::
...
Durant una suma o resta, la mantissa del sumand de menor magnitud es desplaça a la dreta per igualar els exponents, i alguns bits queden fora de la mantissa de $p$ bits. Descartar-los introdueix error mentre que conservar-los tots requeriria un sumador de mida arbitrària. Es demostra que els tres bits $G$, $R$ i $S$ (@sec-bits-de-guarda) basten per obtenir **exactament** el mateix resultat arrodonit que amb tots els bits: $G$ pot incorporar-se a la mantissa en normalitzar, $R$ decideix l'arrodoniment i $S$ (l'OR de la resta) distingeix el cas equidistant.
...
3. **Normalitzar** el resultat, desplaçant la mantissa fins que la part entera sigui 1, ajustant l'exponent en conseqüència; cal comprovar sobreeiximent/subdesbordament.
...
<!-- TODO: PM: Check that this formula is well displayed under LaTeX -->
...
La suma d'enters en Ca2 és associativa: $(a + b) + c = a + (b + c)$ sempre. En coma flotant, aquesta propietat simplement no es compleix a causa dels arrodoniments intermedis.
...
L'**extensió F** de RISC-V afegeix 32 registres de coma flotant de 32 bits, **`f0`–`f31`**, independents dels registres enters de RV32I. L'ABI `ilp32f` assigna noms i rols a cadascun, seguint el mateix esquema de temporals, segurs i paràmetres que els registres enters (@nte-registres-proposit-general).
...
<!-- TODO: PM: figures amb descripcions en anglès o català? -->
...
## RV32F ABI — Pseudoinstrucció `fmv.s`
...
## Conversió entre `float` i `int`: `rtz` vs. arrodoniment
...
```{.s filename="RV32IF"}
truncar:
        fcvt.w.s  a0, fa0, rtz     # a0 = (int) x  [trunca cap a zero, com el cast de C]
        ret
```

Si s'escriu `fcvt.w.s a0, fa0` (sense operand de mode) l'assemblador hi assigna `dyn` (mode dinàmic): la conversió s'arrodoneix segons el mode vigent al camp `frm` del `fcsr` (@nte-fcsr), que per defecte és RNE (al més pròxim, empat al parell). Per a $x = 2{,}5$, això dona resultats diferents:

| Instrucció | Mode | Resultat |
| :--- | :--- | :---: |
| `fcvt.w.s a0, fa0, rtz` | Truncament (cast de C) | $2$ |
| `fcvt.w.s a0, fa0` | `dyn` → RNE per defecte | $2$ (empat, al parell) |

Per a $x = 3{,}5$, en canvi, la diferència és explícita: `rtz` dona $3$; `dyn`/RNE dona $4$ (empat, al parell). La instrucció `fcvt.w.s` trunca només quan se n'indica el mode `rtz`.
:::
