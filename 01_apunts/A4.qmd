...
## Suma, resta i canvi de signe {#sec-suma-resta-i-canvi-de-signe}

Les instruccions de suma (`add`) i resta (`sub`) d'enters ja s'han introduït a @nte-instruccions-aritmetica-entera-rv32i; aquí se'n reprèn l'ús per estudiar-ne el **sobreeiximent** i per definir el **canvi de signe**.

::: {#nte-pseudoinstruccio-neg .callout-note}
## Pseudoinstrucció — `neg` (canvi de signe)

El **canvi de signe** d'un registre s'obté amb la pseudoinstrucció `neg`:

| Pseudoinstrucció | Operació | Expansió |
| :--- | :--- | :--- |
{{< include ../21_riscv/RV32I_pseudo_neg.qmd >}}
:::

Gràcies a l'equivalència entre la resta i la suma amb el negatiu del subtrahend, el maquinari necessita un sol circuit per a la suma i la resta (@sec-resta-ca2).
...
::: {#wrn-sobreeiximent-llenguatges .callout-warning collapse=true}
## Sobreeiximent i llenguatges de programació

La necessitat de detectar el sobreeiximent depèn del llenguatge d'alt nivell. En
llenguatges com Fortran o Ada, quan la suma o resta d'enters produeix sobreeiximent
el programa ha de llançar una excepció, mentre que els sobreeiximents de naturals han
de ser ignorats. En canvi, llenguatges com C estipulen que tots els sobreeiximents siguin
ignorats (per als naturals el resultat es trunca a mòdul $2^n$, i per als enters el
resultat és indefinit).

Pel que fa al maquinari, alguns processadors ofereixen suport per detectar el
sobreeiximent (mitjançant indicadors o instruccions específiques). D'altres no, i
en aquest cas la detecció és responsabilitat del programari (com en el cas de RV32I).
:::
...
### Condicions de sobreeiximent {#sec-condicions-de-sobreeiximent}
...
## Demostració de les condicions de sobreeiximent en Ca2
...
### Detecció del sobreeiximent per maquinari {#sec-deteccio-del-sobreeiximent-per-maquinari}
...
## Detecció de sobreeiximent per maquinari — el sumador amb propagació de l'arrossegament
...
<!-- TODO: Afegir la figura d'un half-adder i la d'un full-adder -->
<!-- TODO: Afegir la figura de la seqüència de full-adders amb la porta XOR per detectar el sobreeiximent en el darrer? -->
:::
...
és a dir, hi ha sobreeiximent si els operands tenen el mateix signe i el resultat té signe diferent.

::: {#tip-sobreeiximent-programari .callout-tip}
## Càlcul de la condició de sobreeiximent per programari
...
<!-- TODO: Aquest "caution" hauria d'estar més endavant, quan es parla de l'extensió M i no pas al principi d'introducció de les multiplicacions -->

::: {#cau-sobreeiximent-extensio-m .callout-caution}
El criteri vist per a la suma i la resta (@cau-ec-no-overflow) s'aplica a tota l'aritmètica entera de RISC-V, inclosa l'extensió M: el resultat es trunca silenciosament als 32 bits de menys pes, sense excepció ni indicador. La detecció, si cal, és responsabilitat del programari (@sec-sobreeiximent-programari).
:::

::: {#wrn-sobreeiximent-naturals .callout-warning collapse=true}
## Sobreeiximent en naturals
...
### Algorisme {#sec-algorisme-multiplicacio}
...
Per evitar guardar tots els productes parcials simultàniament, el circuit els va acumulant a mesura que es calculen en un registre acumulador de $2n$ bits, inicialitzat a zero. En total, el circuit disposa de **tres registres**:
...
::: {#fig-multiplicador-sequencial}
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_multiplicador_sequencial__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_multiplicador_sequencial__original_dark.svg)
:::
:::
...
<!-- TODO: La descripció de l'algorisme no hauria de ser tota en cursives. Els titols del codis no haurien de ser simplemement "pseudocodi". -->

::: {#thm-multiplicacio-sequencial}
Multiplicació seqüencial de naturals de $n$ bits, on $x$ multiplicand, $y$ multiplicador i $z$ el resultat.

```{.default filename="Pseudocodi"}
// Inicialització
MD[n-1:0] = x;  MD[2n-1:n] = 0
MR = y
P = 0
...
::: {#fig-multiplicador-arbre}
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_multiplicador_arbre__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_multiplicador_arbre__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T4_multiplicador_arbre__original_light.svg)
:::
Circuit multiplicador combinacional en arbre.
:::
...
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32M_instruccions_multiplicacio.qmd >}}
...
::: {#wrn-mul-mulh-fusio .callout-warning collapse=true}
## Fusió de `mul` i `mulh`

`mul` i `mulh` (o `mulhu`/`mulhsu`) són instruccions independents: cadascuna calcula internament la multiplicació completa de 64 bits i en retorna una meitat. Per evitar fer el càlcul dues vegades, l'especificació de RISC-V recomana als compiladors que, quan calguin les dues meitats del resultat, posin les dues instruccions consecutivament amb els mateixos operands; això permet al maquinari detectar el patró i fusionar-les en una sola multiplicació de 64 bits.
:::

### Sobreeiximent en la multiplicació {#sec-sobreeiximent-en-la-multiplicacio}
...
## Detecció de sobreeiximent en la multiplicació entera
...
```{.s filename="RV32IM"}
        ...
        bne     t3, t4, sobreeiximent  # sobreeiximent si t3 != ext.ensió de signe de t2
```
:::
...
A la @sec-algorisme-multiplicacio s'ha presentat l'algorisme general per a enters: treballar amb els valors absoluts i ajustar el signe al final. 
...
### Algorisme {#sec-algorisme-divisio}
...
parcial (la comparació es fa com una resta). Si el resultat és negatiu (bit de signe $R_{2n-1} = 1$), el bit de quocient és 0 i es restaura el dividend sumant-hi de nou el divisor; si el resultat és positiu o zero, el bit és 1.

::: {#fig-divisor-sequencial}
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_divisor_sequencial__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_divisor_sequencial__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T4_divisor_sequencial__original_light.svg)
:::
Circuit divisor seqüencial amb restauració.
:::

::: {#thm-divisio-sequencial-restauracio}
Divisió seqüencial de naturals de $n$ bits amb algorisme de restauració, on $x$ és el dividend, $y$ el divisor, $z$ el quocient i $w$ el residu.
...
| Mnemònic | Operands | Operació | Nom | Tipus |
| :--- | :--- | :--- | :--- | :---: |
{{< include ../21_riscv/RV32M_instruccions_divisio.qmd >}}
: {tbl-colwidths="[10,20,35,30,5]"}
:::

`div` i `rem` operen amb enters amb signe. `divu` i `remu` operen amb naturals. `div` i `divu` escriuen el quocient a `rd`; `rem` i `remu`
escriuen el residu.
...
#### Casos especials {#sec-casos-especials}

- **Divisor zero**: el resultat és indefinit. RISC-V no genera excepció. Per convenció, `div` retorna $-1$ i `rem` retorna el dividend.
- **Sobreeiximent en enters**: l'únic cas possible és $x = -2^{31}$, $y = -1$, ja que el resultat exacte $2^{31}$ no és representable en Ca2 amb 32 bits. Per convenció, `div` retorna $-2^{31}$ i `rem` retorna 0.

::: {#tip-divisio-exemple .callout-tip}
## Traducció de divisió i mòdul d'enters
...
### Divisió per potències de 2 {#sec-divisio-per-potencies-de-2}
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_matriu_emmagatzematge__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_matriu_emmagatzematge__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T4_matriu_emmagatzematge__original_light.svg)
...
$$@\texttt{mat[i][j]} = @\texttt{mat[0][0]} + (i \cdot \texttt{NC} + j) \cdot T$$ {#eq-acces-aleatori-matriu}
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_matriu_offset_ij__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_matriu_offset_ij__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T4_matriu_offset_ij__original_light.svg)
...
::: {#imp-ec-mul-adreces .callout-important}
A EC s'usa `mul` en els càlculs d'adreces amb multiplicacions per constants que no són potències de 2. Els 32 bits de més pes s'ignoren perquè qualsevol adreça vàlida en RV32I cap en 32 bits.
:::
...
Si la columna `j` és una constant (per exemple `j = 5`):
...
Si la fila `i` és una constant (per exemple `i = 3`):
...
Els bucles que recorren vectors i matrius sovint admeten optimitzacions que redueixen el nombre d'instruccions executades per iteració. Com s'ha vist a la @sec-extraccio-invariants, l'extracció d'invariants és un prerequisit implícit que cal aplicar sempre abans de qualsevol altra optimització.
...
Passos per aplicar l'accés seqüencial:
...
Versió d'accés aleatori sense optimitzar:
...
Versió d'accés seqüencial optimitzada:
...
La infraestructura de control queda en **4 instruccions per iteració**. Aplicant les optimitzacions #2 i #3 podrem reduir aquest número fins a 2.
:::
...
Partint de la versió #1 amb accés seqüencial:
...
Després d'aplicar la versió #1 amb accés seqüencial, el punter `p` recorre tots els elements del bucle. Si la variable d'inducció `j` ja no s'usa per a res més que controlar el bucle, es pot eliminar: en lloc de comparar `j` amb `NC`, es compara `p` directament amb l'adreça de l'element posterior al darrer.
...
Partint de la versió amb les optimitzacions #1 i #2:
...
::: {.content-visible when-format="html"}
::: {.light-content}
![](/auto_figs/T4_matriu_recorreguts_strides__original_light.svg)
:::
::: {.dark-content}
![](/auto_figs/T4_matriu_recorreguts_strides__original_dark.svg)
:::
:::
::: {.content-visible when-format="pdf"}
![](/auto_figs/T4_matriu_recorreguts_strides__original_light.svg)
