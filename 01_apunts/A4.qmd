...
Els blocs d'aquest tema usen aquestes constants dins d'expressions aritmètiques (`NF*NC*4`, `NC*4`, `mat + 3*NC*4`) per fer visible l'estructura del càlcul d'adreces. **RARS no avalua expressions aritmètiques als operands** (@nte-rars-operands-literals) ni admet `la` amb desplaçament (@imp-ec-la-offset): al laboratori cal escriure-hi el **literal ja calculat**, amb l'expressió al comentari, i substituir `la rd, etiqueta+desplaçament` per la parella `la` + `addi`.
:::
