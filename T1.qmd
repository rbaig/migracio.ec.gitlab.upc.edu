...
## La gestió de la complexitat: Capes i interfícies
...
### Interfícies: Els contractes del sistema
...
* ***Application Programming Interface*** (**API**)**:** Contracte a nivell de codi font. Garanteix la portabilitat del programari entre diferents sistemes si es respecten els estàndards (com **ISO C** o POSIX). **Defineix s'ha d'escriure el codi**.
...
El procés de transformació d’una idea abstracta en un fitxer executable no és un pas directe, sinó una cadena de muntatge (***toolchain***) altament coordinada on intervenen diferents eines de sistema. Aquest flux, que va des de l'escriptura del codi fins a la generació del binari executable, es pot desglossar en quatre (més una) etapes fonamentals on les interfícies API, ABI i ISA actuen com a garanties de compatibilitat:
   ...
   4. **Enllaçat** (*linking*): L'Enllaçador és el pas final i crític de síntesi. La seva funció és combinar el codi objecte generat pel programador amb les biblioteques del sistema, tant estàtiques (`.a`) com dinàmiques (`.so`). Per fer-ho, l'enllaçador ha de tenir un coneixement profund de l'**ISA** (per resoldre salts i adreces de memòria) i de l'**ABI** (per assegurar que totes les peces "parlen" el mateix llenguatge binari), donant com a resultat el Binari executable llest per ser carregat a la memòria.

```{mermaid}
%%| label: fig-flux-compilacio
%%| fig-cap: "Flux de generació d'un programa executable, des de la seva implementació amb codi d'alt nivell, fins a l'obtenció d'un fitxer executable."
%%| fig-width: 100%

flowchart LR
    A["Programador"] -->|"API"| B["Codi font<br/>(.c, .h)"]
    B -->|"Preprocessador"| C["Codi expandit<br/>(.i)"]
    C -->|"Compilador<br/>API + ABI"| D["Assemblador<br/>(.s)"]
    D -->|"Assemblador<br/>ABI + ISA"| E["Objecte\n(.o)"]
    E -->|"Enllaçador<br/>ISA + ABI<br/>.a + .so"| F["Binari<br/>executable"]
```

::: {#tip-gcc .callout-tip}
La [**GCC/Clang toolchain**](https://gcc.gnu.org/) és l'estàndard *de facto* de la indústria per a sistemes oberts.
:::
...
Des del punt de vista del programador interfícies actuen com a **normes de construcció**.
...
* **L'ISA en execució:** És el moment de la veritat. La Unitat de Control del processador llegeix els bits del binari. Si aquests bits no coincideixen amb l'**ISA** del maquinari, el processador no sabrà quines portes lògiques obrir i el sistema petarà (instrucció il·legal).
...
::: {#tbl-impacte-canvi-interficies tbl-colwidths="[15,25,55]" style="width: 90%;"}
...
### La dualitat de l'entorn: *host* vs. *target*
...
::: {#tip-toolchain .callout-tip  appearance="simple"}
...
1. **Gestió de memòria:** Mitjançant els punters, el programador decideix exactament on resideixen les dades. Això és vital en sistemes **bare-metal** per accedir a registres de control del maquinari que estan mapejats en memòria (*Memory-Mapped I/O*).
...
### Traducció: De C al llenguatge de màquina
...
En aquest exemple, el compilador ha seguit les regles de l'**ABI** per decidir que els paràmetres `a` i `b` es passen a través dels registres `a0` i `a1`. La instrucció `add` és una operació definida per la **ISA** de RISC-V que realitza la suma. Finalment, la instrucció `ret` és una pseudoinstrucció que el compilador tradueix a un salt incondicional a l'adreça de retorn.
...
Un **microcontrolador** és un sistema informàtic complet integrat en un sol xip de silici, dissenyat específicament per controlar tasques concretes en sistemes encastats. A diferència d'un **microprocessador** convencional (com el d'un PC), que només conté la Unitat Central de Processament (CPU) i necessita components externs per funcionar, el microcontrolador incorpora dins del mateix encapsulat la memòria (RAM i Flash), els circuits de rellotge i una gran varietat de perifèrics d'Entrada/Sortida (com GPIO, UART o ADC). Les diferències principals són:
...
<!-- TODO estandaritzar mermaid quan bug `height=480` solucionat -->
::: {style="text-align: center;"}
``` {mermaid}
flowchart LR
    A["Host"] -->|"Cable USB"| B["Sonda"]
    B -->|"SWD"| C["Target"]
```
:::

* **Host (PC):**
    * Executa el programa **depurador** ([GDB/OpenOCD](https://openocd.org/)), que, a través de CMSIS-DAP, dona **ordres** a la sonda i en reb les **respostes** a la sonda.
...
``` {mermaid}
flowchart LR
    A["Host"] -->|"Binari"| B["Sonda"]
    B -->|"Escriptura"| C["Target"]
```
:::
...
* **Target (Pi Pico 2):** 
    * Reb i emmagatzema a la memòria permanent (***flash***) el binari.
...
``` {mermaid}
flowchart LR
    A["Host"] <-->|"Control/Monitoratge"| B["Sonda"]
    B <-->|"Control/Monitoratge"| C["Target"]
```
:::
...
| **Reserva de memòria** | `new int[10];` | `malloc(10 * sizeof(int));` | Cal gestionar la mida exacta en bytes. |
| **Alliberar memòria** | `delete[] v;` | `free(v);` | Gestió manual del *heap* ([Tema 4](t4.html)). |
:::
::::

### Llenguatges interpretats

<!-- TODO resecriure per evitar bullet list amb un sol bullet -->

Mentre que C es **compila** (es tradueix tot abans d'executar), altres llenguatges com Python es **interpreten**:

* **Interpretació:** Un programa (l'intèrpret) llegeix el codi font i el tradueix a instruccions de l'ISA *mentre* el programa corre.

**Els llenguatges interpretats no són apropiats per EC** perquè, en general, encara fan molt més opacs els detalls d'arquitectura, que és precisament l'àmbit d'interès de l'assignatura.
...
graph LR
    ...
    Bus{{"Busos (Dades, Adreces, Control)"}}
...
Arquitectura de Von Neumann: CPU, Memòria Principal i Sistema d'E/S connectats per Busos
:::

<!-- TODO estandaritzar mermaid quan bug `height=480` solucionat -->
::: {#fig-von-neumann-simple}
```{mermaid}
%%| fig-width: 3
graph LR
    Bus(Busos)
...
Arquitectura de Von Neumann: presentació simplificada amb Busos com a element central
:::
...
Un nombre natural $x_u$ de $n$ bits es representa com una cadena de bits
$X = X_{n-1} \ldots X_1 X_0$, on cada bit $X_i \in \{0, 1\}$.
...
Donat un natural $x_u$, la seva representació en $n$ bits s'obté dels residus de
dividir successivament $x_u$ entre 2, de menor a major pes.

::: {#tip-natural-representacio .callout-tip}
## Representació d'un natural en binari

**Pregunta:** S'ha depresentar $x_u = 25$ amb 8 bits.
...
Un cop el quocient és zero, les divisions restants només afegeixen zeros a
l'esquerra (extensió de zeros, vegeu @sec-extensio-zeros).
:::
...
Per representar un natural de $n$ bits amb $m > n$ bits, s'afegeixen zeros a
l'esquerra. El valor representat no canvia.
...
## Codificació de nombres enters en base 2

Un nombre enter $x_s$ de $n$ bits es representa com una cadena de bits
$X = X_{n-1} \ldots X_1 X_0$, on el bit de més pes $X_{n-1}$ indica el signe:
$0$ si positiu, $1$ si negatiu.

Els enters positius es codifiquen igual que els naturals equivalents.

### Enters en complement a 2 (Ca2)
...
Per a enters **positius**, el procediment és idèntic al dels naturals
(@sec-extensio-zeros).

Per a enters **negatius**, s'aplica la **regla del canvi de signe**: es
complementen tots els bits i se suma 1.
...
Per representar un enter en Ca2 de $n$ bits amb $m > n$ bits, es replica el
bit de signe $X_{n-1}$ a l'esquerra. El valor representat no canvia.
...
La codificació en excés assigna la representació $000\ldots0_2$ al valor més
negatiu i $111\ldots1_2$ al més positiu, de manera que l'ordre de les
representacions coincideix amb l'ordre numèric dels enters. Això permet
**comparar enters amb el mateix circuit que compara naturals**.
...
A **EC**, la codificació en excés s'utilitza per representar l'exponent dels
nombres en **coma flotant** ([Tema 6](T6.qmd)).
:::
...
El bit de més pes indica el signe ($0$ positiu, $1$ negatiu) i els $n-1$
bits restants representen el valor absolut com un natural.

**Rang:** $x_s \in [-(2^{n-1}-1),\ +2^{n-1}-1]$

**Inconvenients:** doble representació del zero ($+0$ i $-0$) i circuit de
suma diferent al dels naturals. No s'usa en processadors moderns.
:::

::: {#wrn-ca1 .callout-warning collapse=true}
## Enters en complement a 1 (Ca1)

L'enter negatiu oposat de $x_s$ s'obté complementant tots els bits de la
seva representació.

**Rang:** $x_s \in [-(2^{n-1}-1),\ +2^{n-1}-1]$

**Inconvenients:** doble representació del zero i algorisme de suma
diferent al dels naturals. No s'usa en processadors moderns.
:::